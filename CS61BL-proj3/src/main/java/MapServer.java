...
public class MapServer {
    ...
    private static HashMap<String, List<Map<String, Object>>> cache1 = new HashMap<>();
     ...
     */
    public static BufferedImage getMapRaster(
            Map<String, Double> inputParams,
            Map<String, Object> rasteredImageParams) {

        double defaulwidth = ROOT_LRLON - ROOT_ULLON;
        double defaultheight = ROOT_ULLAT - ROOT_LRLAT;

        double inputUllat = inputParams.get("ullat");
        double inputUllon = inputParams.get("ullon");
        double inputLrlat = inputParams.get("lrlat");
        double inputLrlon = inputParams.get("lrlon");
        double inputWidth = inputParams.get("w");
        double inputHeight = inputParams.get("h");

        boolean qusucc = false;

        int depth = 0;
        double distancePerpixel = (inputLrlon - inputUllon) / inputWidth;
        double initialdistancePerpixel = (ROOT_LRLON - ROOT_ULLON) / TILE_SIZE;

        if (initialdistancePerpixel <= distancePerpixel) {
            depth = 0;
        } else {
            while (initialdistancePerpixel > distancePerpixel) {
                initialdistancePerpixel /= 2;
                depth++;
            }
        }
        ...
        //generate QuadTree object and search for images
        QuadTree q = new QuadTree(ROOT_ULLAT, ROOT_ULLON, ROOT_LRLAT, ROOT_LRLON);
        ArrayList<QTreeNode> images = q.testSearch(inputUllat, inputUllon, inputLrlat,
                inputLrlon, depth);

        int rowNum = q.getRow();
        int columnNum = q.getColumn();
        ...
        double raullon = images.get(0).getUpper().getLongitude();
        double raullat = images.get(0).getUpper().getLatitude();
        double ralrlon = images.get(images.size() - 1).getLower().getLongitude();
        double ralrlat = images.get(images.size() - 1).getLower().getLatitude();
        int rawidth = q.getColumn() * TILE_SIZE;
        int raheight = q.getRow() * TILE_SIZE;

        rasteredImageParams.put("raster_ul_lon", raullon);
        rasteredImageParams.put("raster_ul_lat", raullat);
        rasteredImageParams.put("raster_lr_lon", ralrlon);
        rasteredImageParams.put("raster_lr_lat", ralrlat);
        rasteredImageParams.put("raster_width", rawidth);
        rasteredImageParams.put("raster_height", raheight);
        rasteredImageParams.put("depth", depth);
        rasteredImageParams.put("query_success", qusucc);

        BufferedImage result = new BufferedImage(
                rawidth, raheight, //work these out
        ...
        try {
            for (QTreeNode image : images) {
                String name = "img/" + image.getName() + ".png";
                BufferedImage bi;
                ...
                //BufferedImage bi = ImageIO.read(new File("img/1.png"));
    ...
    public static List<Long> findAndDrawRoute(Map<String, Double> routeParams,
        ...
        Point startPoint = new Point(routeParams.get("start_lat"), routeParams.get("start_lon"));
        Point endPoint = new Point(routeParams.get("end_lat"), routeParams.get("end_lon"));

        GraphNode start = g.nearestNode(startPoint);
        GraphNode end = g.nearestNode(endPoint);

        ArrayList<GraphNode> route = g.route(start, end);
        ...
        if (im != null) {
            ...
            double rasterImageullat = (double) rasterImageParams.get("raster_ul_lat");
            double rasterImageullon = (double) rasterImageParams.get("raster_ul_lon");
            double rasterImagelrlat = (double) rasterImageParams.get("raster_lr_lat");
            double rasterImagelrlon = (double) rasterImageParams.get("raster_lr_lon");
            int rasterWidth = (int) rasterImageParams.get("raster_width");
            int rasterHeight = (int) rasterImageParams.get("raster_height");
            int rasterDepth = (int) rasterImageParams.get("depth");

            double rasterImageWidth = rasterImagelrlon - rasterImageullon;
            double rasterImageHeight = rasterImageullat - rasterImagelrlat;

            for (int i = 0; i < route.size() - 1; i++) {
                double x1d = route.get(i).getLon();
                double y1d = route.get(i).getLat();
                double x2d = route.get(i + 1).getLon();
                double y2d = route.get(i + 1).getLat();
                //get pixel coordinate
                int x1 = (int) (rasterWidth * (x1d - rasterImageullon) / rasterImageWidth);
                int y1 = (int) (rasterHeight * (rasterImageullat - y1d) / rasterImageHeight);

                int x2 = (int) (rasterWidth * (x2d - rasterImageullon) / rasterImageWidth);
                int y2 = (int) (rasterHeight * (rasterImageullat - y2d) / rasterImageHeight);

                graphics.drawLine(x1, y1, x2, y2);
            }
        }

        for (GraphNode a: route) {
            list.add(a.getId());
        }
        return list;
    }
    ...
    public static List<String> getLocationsByPrefix(String prefix) {
        ...
        List<String> result = g.getLocationsByPrefix(cleanPrefix);
        return result;
    }
    ...
    public static List<Map<String, Object>> getLocations(String locationName) {
        String cleanPrefix = GraphDB.cleanString(locationName);

        if (cache1.containsKey(locationName)) {
            return cache1.get(locationName);
        }
        ...
        ArrayList<GraphNode> result = g.getLocations(cleanPrefix);
