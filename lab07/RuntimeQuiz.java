public class RuntimeQuiz {
    ...
    public int f2(int n) {
        if (n <= 1) return n;
        f1(new int[n]); //n
        return n + n * f2(n - 1) + n * n * f2(1); //f1(n)+f1(n-1)+...=n^2
    }
    ...
    public int f4(char[] array, int start, int end) {
        if (array.length <= 1 || end <= start) return 1;
        int counter = 0;
        for (int i = start; i < end; i++) {//altogether is n
            if (i == 'a') counter++;
        }
        int mid = start + ((end - start) / 2);
        return counter + f4(array, start, mid) + f4(array, mid + 1, end);//logn lines
    }
    ...
    public void f6(int[] array) {
        for (int i = 1; i < array.length; i++) {//this is n but with conditions, so big O here
            if (array[i] == array[i-1]) {
                System.out.println("Sarah is the potatoest");
                return;//n but has a chance to return eailier
            }
        }
    }
    ...
    public int f7(int[] array, int start, int end) {
        if (array.length <= 1 || end <= start) {//normally ignores this chance for we only consider bit N input
            return 1;
        } else if (array[start] <= array[end]) {
            return f7(array, start + 1, end - 1);//n/2
        } else {
            int tmp = array[start];
            array[start] = array[end];
            array[end] = tmp;
            return f7(array, start + 1, end) + f7(array, start, end - 1)
                    + f7(array, start + 1, end - 1);//3 to the power of N
        }
    }
	...
	//this should be big O but since we need to get and delete all object n, so need to go through the whole list
    public static Runtime remove_object_runtime = Runtime.LINEAR;

    /* remove(DLNode n) */ //we can directly set its next and previous to connect to each other.
