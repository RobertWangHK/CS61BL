public class FixedSizeList implements SimpleList {
    ...
    public void add(int k) {
        ...
        if(count==values.length){
            throw new ListException("out of bound add1");
        }
        values[count]=k;
        count++;
    }
    ...
    public int get(int i) {
        // YOUR CODE HERE
        if(i<0||i>=count){
            throw new ListException("out of bound get");
        }
        return values[i];
    }
    ...
    public void add(int i, int k) {
        if(count==values.length){
            throw new ListException("out of bound add");
        }
        if(i>=count){
            values[count]=k;
        }
        else{
            for (int h = count; h >= i+1; h--) {
            values[h] = values[h-1];
        }
            values[i] = k;
        }
    ...
    public void removeIndex(int i) {
        // YOUR CODE HERE
        if(i<0||i>=count){
            throw new ListException("out of bound removeindex");
        }
